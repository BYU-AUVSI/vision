...
class location_listener:
    def __init__(self):
        ...
        N_targets = 9       #CHANGE THIS TO CORRECT NUMBER OF TARGETS
        self.locations = np.zeros((N_targets,3))
        self.target_counter = np.zeros(N_targets)      #keeps track of how many times each target has been updated (for averaging)

        #target location publishers
        self.targ_pub = rospy.Publisher('pos_t1',FloatList,queue_size=10)

        self.listofpublishers = [0]*N_targets
        for i in range(N_targets):
            self.listofpublishers[i] = rospy.Publisher('pos_'+str(i),FloatList,queue_size=10)
